...
icb_df <-
  tibble(
    ...
    id = query_urls |> slice(which(query_urls$id == "nhs_accident_emergency_april_22"):which(query_urls$id == "nhs_accident_emergency_october_23")) |> pull(id),
    sheet = rep("System Level Data", 19),
    range = rep("B16:AB63", 19),
    date = c(
...
trust_df <-
  tibble(
    ...
    sheet = rep("Provider Level Data", 31),
    range = rep("B16:AB223", 31),
    date = c(
    ...
    col_index = rep(list(c(1, 7, 15, 25, 26)), 31)
  )
